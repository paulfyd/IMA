function [mvf prediction] = me(cur, ref, brow, bcol, search, lambda, outofbound)
...
%
%   The out-of-boundary behavior can be defined using a last parameter:
%   mvf=me(cur, ref, brow, bcol, search, lambda, outofbound)
%   outofbound can be 'sym', 'rep' and 'inp'. Default is 'rep'
%
%   [mvf prediction] = me(...)
...
if ~exist('outofbound','var'), outofbound = 'replicate'; end
switch lower(outofbound)
    case {'rep','replicate','replicated'}
        ref_extended =  padarray(ref,[extension extension],'replicate','both');
    case {'sym','symm','symmetric', 'circular'}
        ref_extended =  padarray(ref,[extension extension],'symmetric','both');
    case {'inp' 'inpaint' 'inpainting'}
        ...
        ref_extended = yuv_ref_extended(:,:,1);        
    otherwise
        warning('ME:EXT','Unrecognized image extension for ou-of-boundary vectors. Using default replicated extension');
        ref_extended =  padarray(ref,[extension extension],'replicate','both');
end
